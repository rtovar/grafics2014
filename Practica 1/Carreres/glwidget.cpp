...
void GLWidget::adaptaObjecteTamanyWidget(Objecte *obj) {
    ...
    float xTrasl = obj->capsa.pmin.x + obj->capsa.a/2.;
    float yTrasl = obj->capsa.pmin.y + obj->capsa.h/2.;
    float zTrasl = obj->capsa.pmin.z + obj->capsa.p/2.;

    mat4 mat = Translate(-xTrasl, -yTrasl, -zTrasl);
    mat4 mat2 = Translate(xTrasl/a, yTrasl/h, -zTrasl/p);

    obj->aplicaTG(mat2*modScalat*mat);
...
void GLWidget::newTerra(float amplaria, float profunditat, float y)
{
    ...
    Terra* terra = new Terra(amplaria,profunditat,y);
    newObjecte(terra);

    /*terra->toGPU(program);
    terra->aplicaTGCentrat(transform);
    terra->draw();*/
...
void GLWidget::resetView()
{
    ...
    // Metode a modificar per a adaptar tots els objectes de l'escena. .... potser ja esta
